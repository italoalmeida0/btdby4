...
async function initBTDby4(wasmSource) {
  ...
  initPromise = (async () => {
    ...
    const wasm = globalThis.__btdby4_wasm_instance;
    if (!wasm) {
    ...
    const instance = {
      ...
      estimateThinkingTokens(encryptedPayload) {
        return wasm.estimateThinkingTokens(encryptedPayload);
      },
      countImageSize(width, height) {
        return wasm.countImageSize(width, height);
      },
      countImageBase64(base64Str) {
        return wasm.countImageBase64(base64Str);
      },
      countImageBytes(bytes2) {
        const u8 = bytes2 instanceof Uint8Array ? bytes2 : new Uint8Array(bytes2);
